...
export function MiniProfile({
  ...
  const filteredRoles = serverRoles.filter(
    (role) =>
  ...
  return (
    <div className="flex flex-col space-y-2 items-center p-3">
      ...
      <h2
        className={
          server_user_profile.server_user?.nickname ? 'text-base' : 'text-lg'
        }
      >
          ...
          <div className="flex flex-col w-full items-center space-y-1">
            {server_user_profile.roles.map((role) => (
              <span
                ...
                style={{
                  border: `1px solid #${
                    !!role.color ? role.color : 'cacacacc'
                  }`,
      ...
      <hr />
      {server_user_profile.id !== user?.id && (
        <form>
          <input
            type="text"
